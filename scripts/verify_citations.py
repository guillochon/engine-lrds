...
so the citations can be checked for existence and relevance.
    ...
    export $(cat .env | tr -d '\r')   # ADS_TOKEN=... (https://ui.adsabs.harvard.edu/user/settings/token)
    ...
    python scripts/verify_citations.py -o out.md     # choose output path (default research/citations_ads.md)
...
ADS title, the author list, year, and the abstract. Read it against the sentence
each key is attached to in the manuscript; existence is not relevance.
"""
...
def main():
    ...
    ap.add_argument("--tex", default=os.path.join(PAPER, "lrd_engine.tex"))
    ap.add_argument("--bib", default=os.path.join(PAPER, "lrd.bib"))
    ap.add_argument("-o", "--out", default=os.path.join(ROOT, "research", "citations_ads.md"))
    ...
    uncited = [k for k in bib if k not in keys]
    if args.all:
        keys += uncited
    ...
    if uncited:
        print(f"WARNING: in the .bib but never cited: {uncited}", file=sys.stderr)
    ...
    with open(args.out, "w", encoding="utf-8") as fh:
        fh.write(f"# ADS records for citations in {os.path.basename(args.tex)}\n\n")
